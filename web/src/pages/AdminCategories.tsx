...
export function AdminCategories() {
  ...
  const [form, setForm] = useState({ dewey_code: '', subject_title_en: '', subject_title_ar: '' })
  ...
  const createMutation = useMutation({
    mutationFn: (d: { dewey_code: string; subject_title_en: string; subject_title_ar?: string }) =>
      admin.categories.create(d),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-categories'] })
      setForm({ dewey_code: '', subject_title_en: '', subject_title_ar: '' })
      setShowForm(false)
    },
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data: d }: { id: string; data: { dewey_code: string; subject_title_en: string; subject_title_ar?: string } }) =>
      admin.categories.update(id, d),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-categories'] })
      setEditingId(null)
      setEditingForm({ dewey_code: '', subject_title_en: '', subject_title_ar: '' })
    },
    onError: (err: { response?: { data?: { message?: string } } }) => {
  ...
  const handleStartEdit = (c: Category) => {
    setEditingId(c._id)
    setEditingForm({ dewey_code: c.dewey_code, subject_title_en: c.subject_title_en, subject_title_ar: c.subject_title_ar || '' })
    setError('')
  }

  const handleSaveEdit = () => {
    if (!editingId || !editingForm.dewey_code.trim() || !editingForm.subject_title_en.trim()) return
      ...
      {showForm && (
        <div className="mb-6 p-4 bg-stone-50 rounded-lg border border-stone-200">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <input
              type="text"
              value={form.subject_title_en}
              onChange={(e) => setForm((p) => ({ ...p, subject_title_en: e.target.value }))}
              placeholder={t('admin.subjectTitle') || 'Subject Title (EN)'}
              className="flex-1 min-w-[150px] px-4 py-2 border border-stone-300 rounded-lg"
            />
            <input
              type="text"
              value={form.subject_title_ar}
              onChange={(e) => setForm((p) => ({ ...p, subject_title_ar: e.target.value }))}
              placeholder={(t('admin.subjectTitleAr') as string) || 'Subject Title (AR)'}
              className="flex-1 min-w-[150px] px-4 py-2 border border-stone-300 rounded-lg text-right"
              dir="rtl"
            />
              <button
                type="button"
        ...
        <table className="w-full">
          <thead className="bg-stone-100">
            <tr>
              ...
              <th className="px-4 py-2 text-right">{(t('admin.subjectAr') as string) || 'Subject (AR)'}</th>
              ...
              <tr key={c._id} className="border-t border-stone-200">
                ...
                <td className="px-4 py-2">
                  {editingId === c._id ? (
                    <input
                      type="text"
                      value={editingForm.subject_title_en}
                      onChange={(e) => setEditingForm((p) => ({ ...p, subject_title_en: e.target.value }))}
                      placeholder={t('admin.subjectTitle') || 'Subject Title (EN)'}
                      className="w-full min-w-[120px] px-3 py-1.5 border border-stone-300 rounded-lg"
                    />
                  ) : (
                    c.subject_title_en
                  )}
                ...
                <td className="px-4 py-2 text-right" dir="rtl">
                  {editingId === c._id ? (
                    <input
                      type="text"
                      value={editingForm.subject_title_ar}
                      onChange={(e) => setEditingForm((p) => ({ ...p, subject_title_ar: e.target.value }))}
                      placeholder={(t('admin.subjectTitleAr') as string) || 'Subject Title (AR)'}
                      className="w-full min-w-[120px] px-3 py-1.5 border border-stone-300 rounded-lg text-right"
                      dir="rtl"
                    />
                  ) : (
                    c.subject_title_ar || '—'
                    ...
                    <>
                      <button
                        ...
                        disabled={
                          ...
                          !editingForm.subject_title_en.trim()
                        }
